...
function Slackbot(configuration) {
    ...
    slack_botkit.configureSlackApp = function(slack_app_config, cb) {
        ...
        if (!slack_app_config || !slack_app_config.clientId ||
            ...
            slack_botkit.config.oauthVersion = slack_app_config.oauthVersion || 'v1';
    ...
    slack_botkit.getAuthorizeURL = function(team_id, query) {
        ...
        var oauthAuthorize = (slack_botkit.config.oauthVersion === 'v2') ? '/oauth/v2/authorize' : '/oauth/authorize';

        var url = api_root + oauthAuthorize + '?client_id=' +
            slack_botkit.config.clientId + '&scope=' + scopes.join(',') + '&state=' + (query.state ? query.state : `botkit-${randomUUID()}`);
    ...
    slack_botkit.createOauthEndpoints = function(webserver, callback) {
        ...
        var oauth_access = function(options, cb) {
            call_api((options.oauthVersion === 'v2') ? 'oauth.v2.access' : 'oauth.access', options, cb);
        };
        ...
        webserver.get('/login', function(req, res) {
            res.redirect(slack_botkit.getAuthorizeURL(null, req.query));
        });
        ...
        webserver.get('/oauth', function(req, res) {
            ...
            var opts = {
                ...
                oauthVersion: slack_botkit.config.oauthVersion,
                code: code
            };
                    ...
                    auth_test({token: auth.access_token}, function(err, identity) {
                        ...
                        if (err) {
                            ...
                            slack_botkit.findTeamById(identity.team_id, function(err, team) {
                                ...
                                } else if (auth.token_type === 'bot') { // oauthVersion v2

                                    team.bot = {
                                        token: auth.access_token,
                                        user_id: auth.bot_user_id,
                                        createdBy: auth.authed_user.id,
                                        app_token: auth.access_token,
                                        scopes,
                                    };
                                    slack_botkit.trigger('create_bot', [bot, team.bot]);
