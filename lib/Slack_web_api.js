...
module.exports = function(bot, config) {
    ...
    const client = http2.connect(api_root)
    client.on('error', (err) => console.error(new Error(err)));
    ...
    function postJSON(url, body, cb) {
        ...
        // const result = zlib.gzipSync(buffer, { level: zlib.constants.Z_BEST_COMPRESSION, memLevel: zlib.constants.Z_BEST_COMPRESSION });
        const req = client.request({
            [http2.constants.HTTP2_HEADER_SCHEME]: "https",
            [http2.constants.HTTP2_HEADER_METHOD]: http2.constants.HTTP2_METHOD_POST,
            [http2.constants.HTTP2_HEADER_PATH]: `/api/${url}`,
            // 'Content-Encoding': 'gzip',
            "Content-Type": "application/json; charset=utf-8",
            'Accept-Encoding': 'gzip',
            "Content-Length": buffer.length,
            'authorization' : `Bearer ${config.token}`, // apparently sometimes there's body.token? when?
        });
        
        const data = [];
        const gunzip = zlib.createGunzip();
        gunzip.on('data', (d) => data.push(d));
        gunzip.on('end', () => {
            // TODO: catch + return error as first param
